...
function ParagraphBlock({ text, compact, highlightQuery }: { text: string; compact?: boolean; highlightQuery?: string }) {
  ...
  if (lines.length === 1 && isLikelyHeading(lines[0])) {
    return (
      <p className={`cn-text font-semibold text-slate-900 dark:text-slate-50 ${compact ? "mb-2 text-sm" : "mb-3 text-base"}`}>
        <SearchHighlightedText text={lines[0]} query={highlightQuery} />
      </p>
    );
  }
  ...
  if (allListItems) {
    return (
      <ul className={`cn-text list-none space-y-1 ${compact ? "mb-2" : "mb-4"}`}>
        {lines.map((line, i) => (
          <li key={`li-${i + 1}`} className="text-sm leading-7 text-slate-700 dark:text-slate-300">
            <SearchHighlightedText text={line} query={highlightQuery} />
          </li>
        ))}
      </ul>
    );
  ...
  return (
    <div>
      {blocks.map((block, i) => {
        ...
        return <ParagraphBlock key={`g-${i}`} text={block.text} compact={compact} highlightQuery={highlightQuery} />;
      })}
            ...
            {footnotes.map((note, i) => (
              <div key={`${note.marker}-${i}`} className="flex gap-2 text-sm leading-6 text-slate-600 dark:text-slate-300">
                <span className="flex-shrink-0 font-semibold text-slate-500 dark:text-slate-400">
                  <SearchHighlightedText text={note.marker} query={highlightQuery} />
                </span>
                <span><SearchHighlightedText text={note.content} query={highlightQuery} /></span>
              </div>
            ))}
          </div>
        </section>
